...
(dollars per volatility point per contract, at sample-mean position Greeks), so
$|\mathcal V_{\mathrm{net}}| = 2.0$ against $\mathcal V_{\mathrm{diff}} = 48.0$: the surviving
level exposure is 4.2\% of the differential, or 8.3\% of the slope bucket's coefficient
$\tfrac12\mathcal V_{\mathrm{diff}}$. Because level innovations are in addition only weakly
signed relative to the fade, the level bucket's \emph{signed} P\&L contribution is $-19\%$ of
net---opposite-signed and one-seventh the slope bucket's magnitude. \emph{The delta-matched risk reversal prices the skew, not the
level.}
...
Empirically $\Va_p = -0.756$ and $\Va_c = +1.560$ at sample-mean position Greeks, giving
